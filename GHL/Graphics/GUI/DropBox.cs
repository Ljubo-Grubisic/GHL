...
namespace GHL.Graphics
{
    public class DropBox
    {
        ...
        public DropBox(Vector2f position, Vector2f size, List<string> displayedStrings)
        {
            ...
            DropBoxButton = new Button(Position, Size, "", TextFont, CharacterSize);

            DropBoxArrow = new Arrow(new Vector2f(), 20, Arrow.ArrowState.Down) { OutlineThickness = 3f };
            DropBoxArrow.PositionX = Position.X + Size.X - DropBoxArrow.DeltaSize.X - 2;
            DropBoxArrow.PositionY = Position.Y + Size.Y / 2 + DropBoxArrow.DeltaSize.Y / 2;

            for (int i = 0; i < DisplayedStrings.Count; i++)
            {
                DropBoxItemsButton.Add(new Button(Position + new Vector2f(0, Size.Y * (i + 1)), Size, DisplayedStrings[i], TextFont, CharacterSize));
            }
        }
        ...
        public void Update(Vector2i mousePos)
        {
            ...
            if (MathHelper.IsMouseInRectangle(DropBoxButton, mousePos) && mouseState)
            {
                if (Lock)
                {
            ...
            if (DropBoxArrow.State == Arrow.ArrowState.Up)
            {
                ...
                if (MathHelper.IsMouseInRectangle(Position + new Vector2f(0, Size.Y), Size + new Vector2f(0, Size.Y * (DisplayedStrings.Count - 1)), mousePos) && mouseState)
                {
                    if ((mousePos.Y - (Position.Y + Size.Y)) / Size.Y % 1 != 0)
                    {
                        Index = (uint)((mousePos.Y - (Position.Y + Size.Y)) / Size.Y) + 1;
                        DropBoxArrow.State = Arrow.ArrowState.Down;
                    }
                }
            ...
            if (Index != 0)
            {
                DropBoxButton.DisplayedString = DisplayedStrings[(int)Index - 1];
            }
        }
        ...
        private void DisplayedStringsChanged()
        {
            ...
            foreach (string displayedString in DisplayedStrings)
            {
                DisplayedStringsBuffer.Add(displayedString);
            }
        }
        ...
        private void UpdateDropBoxItemsButtonsPosition()
        {
            ...
            {
                DropBoxItemsButton[i].Position = Position + new Vector2f(0, Size.Y * (i + 1));
                DropBoxItemsButton[i].Size = Size;
            }
        }
        ...
        private class Arrow
        {
            ...
            public Arrow(Vector2f position, float size, ArrowState state)
            {
                ...
                lines[0] = new Line(Position, Position) { OutlineThickness = outlineThickness, OutlineColor = Color };
                lines[1] = new Line(Position, Position) { OutlineThickness = outlineThickness, OutlineColor = Color };
                DeltaSize = new Vector2f(size * (float)Math.Cos(MathHelper.DegreesToRadians(45)), size * (float)Math.Sin(MathHelper.DegreesToRadians(45)));
            ...
            {
                lines[0].Position0 = Position;
                lines[1].Position0 = Position;
                DeltaSize = new Vector2f(size * (float)Math.Cos(MathHelper.DegreesToRadians(45)), size * (float)Math.Sin(MathHelper.DegreesToRadians(45)));

                if (State == ArrowState.Up)
                {
                    lines[0].Position1 = Position + new Vector2f(-size * (float)Math.Cos(MathHelper.DegreesToRadians(45)), size * (float)Math.Sin(MathHelper.DegreesToRadians(45)));
                    lines[1].Position1 = Position + new Vector2f(size * (float)Math.Cos(MathHelper.DegreesToRadians(45)), size * (float)Math.Sin(MathHelper.DegreesToRadians(45)));
                }
                else
                {
                    lines[0].Position1 = Position + new Vector2f(-size * (float)Math.Cos(MathHelper.DegreesToRadians(45)), -size * (float)Math.Sin(MathHelper.DegreesToRadians(45)));
